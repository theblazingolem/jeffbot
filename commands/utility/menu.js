...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('messageid')
                .setDescription('ID of an existing message to update (optional)')
                .setRequired(false)),
    ...
    async execute(interaction) {
        try {
            ...
            const welcomeMsg = menuData.find(item => item.id === 'welcome-msg');
            const messageId = interaction.options.getString('messageid');

            if (!welcomeMsg) {
                return interaction.reply({
                    content: 'Welcome message configuration not found in menu-data.json.',
                ...
                if (comp['action-row']) {
                    ...
                    if (comp.type === 'string-select-menu') {
                        ...
                        comp.options.forEach(opt => {
                            ...
                            if (opt.emoji_id) {
                                selectMenu.options[selectMenu.options.length - 1].setEmoji({
