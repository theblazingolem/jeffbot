            ...
            subcommand
                ...
                .addStringOption((option) =>
                    option
                        .setName("reason")
                        .setDescription(
                            "The reason for removing the timeout (optional)"
                        )
                        .setRequired(false)
                )
        ),
    ...
    async execute(interaction) {
        try {
            ...
            if (!targetMember.moderatable) {
                await interaction.reply({
                    content:
                        "I cannot moderate this user. They might have higher permissions than me.",
                    flags: MessageFlags.Ephemeral,
                });
                return;
            }
...
async function handleRemoveTimeout(
    ...
    await targetMember.timeout(
        null,
        `Timeout removed by ${interaction.user.tag}: ${reason}`
    );
    await interaction.reply({
