const { Events, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder } = require('discord.js');
...
function findInteractionHandler(customId, value = null) {
    ...
    for (const item of menuData) {
        ...
        for (const component of item.components) {
            // For buttons
            if (component.type === 'button' && component.custom_id === customId) {
                console.log(`Found button handler for ${customId} in menuData`);
                return component.onInteraction;
            }

            // For select menus
            if (component.type === 'string-select-menu' && component.custom_id === customId) {
                ...
                if (value && component.options) {
                    for (const option of component.options) {
                        if (option.value === value) {
                            console.log(`Found option handler for ${customId}:${value} in menuData`);
    ...
    if (commandData && commandData.components) {
        for (const component of commandData.components) {
            // For buttons
            if (component.type === 'button' && component.custom_id === customId) {
                console.log(`Found button handler for ${customId} in commandData`);
                return component.onInteraction;
            }
        }
    }
...
function createSelectMenu(menuConfig) {
    ...
    menuConfig.options.forEach(opt => {
        ...
        if (opt.emoji_id || opt.emoji_name) {
            ...
            if (opt.emoji_animated !== undefined) emoji.animated = opt.emoji_animated;
...
function createButton(buttonConfig) {
    ...
    if (buttonConfig.emoji_id || buttonConfig.emoji_name) {
        ...
        if (buttonConfig.emoji_animated !== undefined) emoji.animated = buttonConfig.emoji_animated;
    ...
    async execute(interaction) {
        try {
            ...
            // Handle button interactions
                ...
                const originalComponents = interaction.message.components.map(row => {
                    ...
                    row.components.forEach(component => {
                        if (component.type === 3) { // StringSelectMenu type
                            newRow.addComponents(StringSelectMenuBuilder.from(component));
                        } else if (component.type === 2) { // Button type
                ...
                if (customId === 'show-retired-staff' && interaction.guild) {
                    try {
                        ...
                        console.log("Fetching guild members for retired staff list...");
                        // Fetch all guild members
                        const members = await interaction.guild.members.fetch();
                        console.log(`Fetched ${members.size} members from the guild.`);

                        // Fetch the retired staff role
                        const retiredStaffRole = interaction.guild.roles.cache.get('1349062812722397305');
                        if (!retiredStaffRole) console.log("Retired Staff role not found in cache");
                        ...
                        if (retiredStaffRole) {
                            const retiredStaffMembers = members.filter(m => m.roles.cache.has(retiredStaffRole.id));
                            console.log(`Found ${retiredStaffMembers.size} members with Retired Staff role`);
                        ...
                        console.error('Error handling retired staff button:', error);
                        await interaction.followUp({
                            content: 'There was an error fetching retired staff information: ' + error.message,
                            flags: MessageFlags.Ephemeral
                        });
                    }
                    return;
                ...
                if (handler) {
                    ...
                    if (handler.type === 'message') {
                        try {
                            console.log(`Sending message for button ${customId}`);
                            ...
                            console.error(`Error sending button response ${customId}:`, error);
                        }
                    }
                    else {
                        console.log(`Unhandled interaction handler type: ${handler.type}`);
                        try {
                            // Use deferUpdate to avoid the "edited" label
                            await interaction.deferUpdate();
                            await interaction.followUp({
                            ...
                            console.error('Error sending unsupported type message:', error);
                        }
                    }
                }
                else {
                    ...
                    try {
                        ...
                        console.error('Error sending not configured message:', error);
                    }
                }
            }
            // Handle select menu interactions
                ...
                console.log(`Processing select menu: ${customId}, selected: ${selectedValue}`);
                    ...
                    row.components.forEach(component => {
                        if (component.type === 3) { // StringSelectMenu type
                            ...
                            if (component.customId === customId) {
                                ...
                                const originalMenu = menuData.find(item =>
                                    item.components?.some(comp =>
                                        comp.custom_id === customId
                                    )
                                )?.components?.find(comp => comp.custom_id === customId);

                                if (originalMenu && originalMenu.placeholder) {
                                    newMenu.setPlaceholder(originalMenu.placeholder);
                                } else {
                                    newMenu.setPlaceholder('Select an option');
                                }
                            }
                ...
                if (selectedValue === 'staff-info' && interaction.guild) {
                    try {
                        ...
                        // Fetch the roles
                        const adminRole = interaction.guild.roles.cache.get('862616575890030592');
                        const seniorStaffRole = interaction.guild.roles.cache.get('867964544717295646');
                        const staffRole = interaction.guild.roles.cache.get('842763148985368617');
                        const trialStaffRole = interaction.guild.roles.cache.get('842742230409150495');

                        if (!adminRole) console.log("Admin role not found in cache");
                        if (!seniorStaffRole) console.log("Senior Staff role not found in cache");
                        if (!staffRole) console.log("Staff role not found in cache");
                        if (!trialStaffRole) console.log("Trial Staff role not found in cache");

                        // Build the content
                        let content = '# STAFF\nhierarchy of staff in the server\n\n';

                        // Add administrators
                        content += `**Administrators** (${adminRole ? adminRole.toString() : '<@&862616575890030592>'})\n`;
                        content += '- <@732177983741362256>\n';
                        content += '- <@1038453964812861440>\n\n';

                        // Add senior staff
                        content += `**Senior Staff** (${seniorStaffRole ? seniorStaffRole.toString() : '<@&867964544717295646>'})\n`;

                        if (seniorStaffRole) {
                            const seniorStaffMembers = members.filter(m => m.roles.cache.has(seniorStaffRole.id));
                            console.log(`Found ${seniorStaffMembers.size} members with Senior Staff role`);
                        ...
                        // Add staff
                        content += `**Staff** (${staffRole ? staffRole.toString() : '<@&842763148985368617>'})\n`;

                        if (staffRole) {
                            const staffMembers = members.filter(m => m.roles.cache.has(staffRole.id));
                            console.log(`Found ${staffMembers.size} members with Staff role`);
                        ...
                        // Add trial staff
                        content += `**Trial Staff** (${trialStaffRole ? trialStaffRole.toString() : '<@&842742230409150495>'})\n`;

                        if (trialStaffRole) {
                            const trialStaffMembers = members.filter(m => m.roles.cache.has(trialStaffRole.id));
                            console.log(`Found ${trialStaffMembers.size} members with Trial Staff role`);
                        ...
                        const row = new ActionRowBuilder()
                            .addComponents(retiredStaffButton);
                        ...
                        await interaction.followUp({
                            content: 'There was an error fetching staff information: ' + error.message,
                            flags: MessageFlags.Ephemeral
                        });
                    }
                    return;
                ...
                if (handler) {
                    ...
                    if (handler.type === 'message') {
                        try {
                            console.log(`Sending message for ${customId}:${selectedValue}`);

                            // Update with the original components to reset the placeholder
                            await interaction.update({ components: originalComponents }).then(async () => {
                                await interaction.followUp({
                                    content: handler.content,
                                    flags: MessageFlags.Ephemeral
                                });
                            ...
                            console.error(`Error sending select menu response ${customId}:${selectedValue}:`, error);
                        }
                    }
                    else {
                        console.log(`Unhandled interaction handler type: ${handler.type}`);
                    ...
                    console.log(`No handler found for select menu: ${customId}, value: ${selectedValue}`);
                    try {
                        // Update with the original components to reset the placeholder
                        await interaction.update({ components: originalComponents }).then(async () => {
                            await interaction.followUp({
                                content: 'This select menu option is not configured.',
                                flags: MessageFlags.Ephemeral
                            });
                        });
                    } catch (error) {
            ...
            console.error(`Error processing interaction (${interaction.type} - ${interaction.customId || interaction.commandName}):`, error);
